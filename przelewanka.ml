...
(** funkcja sprawdzająca czy możliwe jest uzyskanie sytuacji z wejścia
...
let canBeDone input = 
  ...
  if emptyOrFull then
    ...
    let doesntDivideAll = Array.exists (fun (_, y) -> 
		y mod inputGCD <> 0) input in
    not doesntDivideAll
  else false;;

(** funkcja szukająca wyniku na podstawie par 
  * [stan, liczba_krokow] w kolejce [que] *)
  ...
  if Hashtbl.mem states state then -1
    ...
    if state = y then steps
      ...
      begin
         ...
         * napełnionej do pewnego stopnia (ale nie do pełna) do innej 
         * szklanki (która nie jest zapełniona) do kolejki *)
        for i = 0 to (n - 1) do
          for j = 0 to (n - 1) do
...
(** funkcja wyznaczająca dla tablicy par (x, y) długości n, w ilu krokach 
  ...
  * że w i-tej szklance znajduje się yi wody. 
  * Jeśli jest to nieosiągalne, funkcja zwraca -1 *)
    ...
    if x = y then 
      ...
      if canBeDone input then
        begin
          ...
          while (not (Queue.is_empty !que)) && !result = - 1 do
            result := backtrack states que x y n
          done;
          !result;
        end
      else -1;;